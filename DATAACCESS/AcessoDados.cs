...
namespace DATAACESS
{
    public class AcessoDados
    {
        ...
        public DataTable ExecutarConsulta(CommandType cmdoType,
                                            string textosql)
        {
            ...
            try
            {
                ...
                // Abrir a conexão
                sqlConnection.Open();
                // Cria o comando que vai levar as informações  até o BD
                SqlCommand sqlCommand = sqlConnection.CreateCommand();
                sqlCommand.CommandType = cmdoType;
                sqlCommand.CommandText = textosql;
                sqlCommand.CommandTimeout = 3600;
                foreach (SqlParameter sqlParameter in SqlParameterCollection)
                {
                    sqlCommand.Parameters.Add(new SqlParameter(
                        sqlParameter.ParameterName, sqlParameter.Value));
                }
                // Criar um adaptador
